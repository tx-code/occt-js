...
Start the next milestone with `/gsd-new-milestone` once the next runtime-first package goal is defined.

## Scope Reminder

`occt-js` stays centered on the runtime-first Wasm carrier.
New milestones should preserve the root package, package-first SDK/docs surfaces, packaged typings, and `npm run test:release:root` as the authoritative release boundary.
